...
void drawPolygonRotation(double angleDeg)
{
    glBegin(GL_POLYGON);
    glColor3f(0.0, 0.0, 1.0);
    ...
    for (int i = 0; i < edges; i++)
    {
        double newX = (pntX[i] * cos(angleRad)) - (pntY[i] * sin(angleRad));
        double newY = (pntX[i] * sin(angleRad)) + (pntY[i] * cos(angleRad));
